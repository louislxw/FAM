...
/********** Floating-point input/output **********/
	...
	int in_size = (P-1)*L + Np; // P*L = 2048 or (P-1)*L + Np = 2240
	int out_size = Np*P/2; // 4096
	...
	for (int i = 0; i <= (in_size); i++) { // 2241
		if (i <= P*L) {
			...
			i_in_float.write(stream_i_in.data);
			q_in_float.write(stream_q_in.data);
		}
		else {
			stream_i_in.data = 0;
			stream_q_in.data = 0;
...
/*	float fixed_square = 0, float_square = 0, error = 0, error_sum = 0, accuracy = 0, dif_square = 0;
	...
	for (int i = 0; i < P; i++) {
    	for (int j = 0; j < Np; j++) {
    		stream_i_out.data = i_pre_out.read();
    		stream_q_out.data = q_pre_out.read();
    		stream_i_out_float.data = float (i_pre_out_float.read());
    		stream_q_out_float.data = float (q_pre_out_float.read());
    		if (i == 0) {
    			printf("i_out[%d][%d] = %f, q_out[%d][%d] = %f\n", i, j, (float)stream_i_out.data, i, j, (float)stream_q_out.data);
    			printf("i_out_f[%d][%d] = %f, q_out_f[%d][%d] = %f\n", i, j, (float)stream_i_out_float.data, i, j, (float)stream_q_out_float.data);
    		}
...
//    		fixed_square = float (stream_i_out.data * stream_i_out.data) + float (stream_q_out.data * stream_q_out.data);
//    		float_square = stream_i_out_float.data * stream_i_out_float.data + stream_q_out_float.data * stream_q_out_float.data;
//    		if (fixed_square != 0 || float_square !=0) {
//    			if ( sqrt(fixed_square) > sqrt(float_square) ) {
//    				error = (sqrt(fixed_square) - sqrt(float_square)) / sqrt(fixed_square);
//    			}
//    			else {
//    				error = (sqrt(float_square) - sqrt(fixed_square)) / sqrt(float_square);
//    			}
//    			error_sum = error_sum +  error;
//    		}
    	}
    }
	RMSE = sqrt(error)/(P*Np);
//	accuracy = (1.0 - error_sum/(P*Np)) * 100;
//	printf("Error_sum = %f,  Accuracy = %f\%\n", (float)error_sum, (float)accuracy);
...
/*	float error = 0, alpha_error = 0, alpha_error_max = 0, RMSE = 0;
	for (int i = 0; i < out_size; i++) {
    		...
    		printf("alpha_out[%d] = %d\n", i, stream_alpha_out.data);
...
//	AXIS_OUT axis_q_out("axis_q_out_stream");
	...
	AXIS_OUT axis_alpha_out("axis_alpha_out_stream");
	model_wrapper(i_in, q_in, axis_alpha_out);
    for (int i = 0; i < out_size; i++) {
		stream_alpha_out = axis_alpha_out.read();
        printf("alpha_out[%d] = %x, TLAST = %d\n", i, stream_alpha_out.data, (unsigned)stream_alpha_out.last);
    }
