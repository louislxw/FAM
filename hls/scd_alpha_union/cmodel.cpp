...
#pragma HLS interface ap_fifo port=fft_config
// #pragma HLS interface ap_fifo port=fft_config,fft_status
...
#pragma HLS PIPELINE II=1
		...
		int Np = 256;
		single_fixed_t1 ibuf[256];
		single_fixed_t1 qbuf[256];
...
// #pragma HLS unroll factor=64
	...
	/*** Y <- X[i] .* conjugate(X[j]) ***/
	for (int k = 0; k < Np; k++){ // original (X) row loop
...
//	bit_reverser_256(i_frame_out, q_frame_out, i_rev_256_out, q_rev_256_out, 32); // Step 2.1
...
#pragma HLS DATAFLOW
    ...
    fft_32(i_rev_32_out, q_rev_32_out, i_out, q_out, 65536); // Step 4.2.2
