...
void HALFFIRE_ACT(
	...
	stream<ap_uint<expand3x3_Cout*expand3x3_Abit> >& out,

	const unsigned reps = 1)
{
#pragma HLS DATAFLOW
	stream<ap_uint<squeeze_Cout*squeeze_Abit> > squeeze_out("squeeze_out");
	CONV2D_1x1_ACT_NoP<squeeze_Din, squeeze_Cin, squeeze_Cout, squeeze_Ibit, squeeze_Wbit, squeeze_Mbit, squeeze_Abit, squeeze_MVTU_InP, squeeze_MVTU_OutP, ScaleBits, FactorScaleBits>
	(in, squeeze_weights, squeeze_factorA, squeeze_factorB, squeeze_out, reps);	

    /*
	stream<ap_uint<squeeze_Cout*squeeze_Abit> > m_squeeze_out("monitor_squeeze_out");
    for (int i = 0; i < squeeze_Din*squeeze_Din; i++) {
        ap_uint<squeeze_Cout*squeeze_Abit> v = squeeze_out.read();
        printf("[squeeze_out_fire1] %llx",(uint64_t)v);
        printf("\n");
		m_squeeze_out.write(v);
	}
    for (int i = 0; i < squeeze_Din*squeeze_Din; i++) {
        ap_uint<squeeze_Cout*squeeze_Abit> v = squeeze_out.read();
        for (int c = 0; c < squeeze_Cout; c++) {
            printf("%d,",(unsigned)v( (c+1)*squeeze_Abit-1, c*squeeze_Abit ));
        }
        printf("\n");
        m_squeeze_out.write(v);
    }
    */
	...
	CONV2D_ACT_NoP<expand3x3_K, expand3x3_S, expand3x3_Din, expand3x3_Cin, expand3x3_Cout, expand3x3_Ibit, expand3x3_Wbit, expand3x3_Mbit, expand3x3_Abit, expand3x3_MVTU_InP, expand3x3_MVTU_OutP, ScaleBits, FactorScaleBits>
	(squeeze_out, expand3x3_weights, expand3x3_factorA, expand3x3_factorB, out, reps);
}

// Ternary-weights (2b) half-fire module
template <	unsigned squeeze_K,
			unsigned squeeze_S,
			unsigned squeeze_Din,
			unsigned squeeze_Cin,
			...
			unsigned ScaleBits,
			unsigned FactorScaleBits>
void TW_HALFFIRE_ACT(
	stream<ap_uint<squeeze_Cin*squeeze_Ibit> >& in,

	const ap_uint<squeeze_MVTU_InP*2> squeeze_weights[squeeze_MVTU_OutP][((squeeze_Cin*squeeze_K*squeeze_K)/squeeze_MVTU_InP)*(squeeze_Cout/squeeze_MVTU_OutP)],
    const ap_int<TWScalebit> squeeze_Wp,
    const ap_int<TWScalebit> squeeze_Wn,
	const ap_int<squeeze_Mbit> squeeze_factorA[squeeze_MVTU_OutP][squeeze_Cout/squeeze_MVTU_OutP],
	const ap_int<squeeze_Mbit> squeeze_factorB[squeeze_MVTU_OutP][squeeze_Cout/squeeze_MVTU_OutP],

	const ap_uint<expand3x3_MVTU_InP*2> expand3x3_weights[expand3x3_MVTU_OutP][((expand3x3_Cin*expand3x3_K*expand3x3_K)/expand3x3_MVTU_InP)*(expand3x3_Cout/expand3x3_MVTU_OutP)], 
    const ap_int<TWScalebit> expand_Wp,
    const ap_int<TWScalebit> expand_Wn,
	const ap_int<expand3x3_Mbit> expand3x3_factorA[expand3x3_MVTU_OutP][expand3x3_Cout/expand3x3_MVTU_OutP],
	const ap_int<expand3x3_Mbit> expand3x3_factorB[expand3x3_MVTU_OutP][expand3x3_Cout/expand3x3_MVTU_OutP],
...
#pragma HLS DATAFLOW
	stream<ap_uint<squeeze_Cout*squeeze_Abit> > squeeze_out("squeeze_out");
	TW_CONV2D_1x1_ACT_NoP<squeeze_Din, squeeze_Cin, squeeze_Cout, squeeze_Ibit, TWScalebit, squeeze_Mbit, squeeze_Abit, squeeze_MVTU_InP, squeeze_MVTU_OutP, ScaleBits, FactorScaleBits>
	(in, squeeze_weights, squeeze_Wp, squeeze_Wn, squeeze_factorA, squeeze_factorB, squeeze_out, reps);	
	...
	TW_CONV2D_ACT_NoP<expand3x3_K, expand3x3_S, expand3x3_Din, expand3x3_Cin, expand3x3_Cout, expand3x3_Ibit, TWScalebit, expand3x3_Mbit, expand3x3_Abit, expand3x3_MVTU_InP, expand3x3_MVTU_OutP, ScaleBits, FactorScaleBits>
	(m_squeeze_out, expand3x3_weights, expand_Wp, expand_Wn, expand3x3_factorA, expand3x3_factorB, out, reps);
}
