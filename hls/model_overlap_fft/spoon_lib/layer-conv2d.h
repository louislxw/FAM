...
#pragma HLS DATAFLOW
    ...
    /*
	stream<ap_uint<MVTU_InP*Ibit> > m_swu_out_reduced("swu_out_reduced");
    for (int i = 0; i < 9; i++) {
        ap_uint<MVTU_InP*Ibit> v = swu_out_reduced.read();
        printf("pixel in = %d\n",(unsigned)v);
        m_swu_out_reduced.write(v);
    }
    */
...
template <	unsigned Din,
			...
			unsigned MVTU_InP,
			unsigned MVTU_OutP>
void CONV2D_1x1_NOACT_NoP_GAP( // GAP = global-average pooling
	stream<ap_uint<Cin*Ibit> >& in, 
	const ap_uint<MVTU_InP*Wbit> weights[MVTU_OutP][(Cin/MVTU_InP)*(Cout/MVTU_OutP)], 
...
#pragma HLS DATAFLOW
    ...
    GlobalAvgPool_Fused<Dout*Dout,Mbit,PoolAccBit,Cout,MVTU_OutP>(out_raw, out, reps);
}
