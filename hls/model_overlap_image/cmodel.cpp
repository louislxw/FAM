...
*/
	...
	for (int r = 0; r < num_slices; r++) {
		ap_int<FFT_W*2> acc[64];
		for (int i = 0; i < 256; i++) {
#pragma HLS PIPELINE II=1
			for (int j = 0; j < 64; j++) {
#pragma HLS UNROLL
				if (i == 0) {
					acc[j] = (ap_int<FFT_W*2>)fft_in.read();
				} else {
					acc[j] += (ap_int<FFT_W*2>)fft_in.read();
				}
			}
		...
		for (int i = 0; i < 64; i++) {
#pragma HLS UNROLL
			macro_slice_out.write((ap_int<FFT_W>)(acc[i]/256));
		}
	}
}
...
void normalizer(
	stream<ap_int<FFT_W> >& macro_slice_in,
	stream<ap_uint<PIXEL_W> >& buffer_out,
	const unsigned num_slices
){
	for (int i = 0; i < num_slices; i++) {
		for (int j = 0; j < 64; j++) {
	#pragma HLS UNROLL
			ap_int<FFT_W> v = macro_slice_in.read();
			v = v >> 1;
			v = (v << 8) - v; // multiply by 255
			if (v > 255) {
				buffer_out.write(255); // img_out.write(255)
			} else {
				buffer_out.write(0); // img_out.write(0)
			}
		}
	}
}
...
#pragma HLS PIPELINE II=1
		//stream<ap_int<FFT_W> > buffer_out("buffer_out_stream");
/*		for (int j = 0; j < 64; j++) {
#pragma HLS UNROLL
			ap_int<FFT_W> v = macro_slice_in.read();
			v = v >> 1;
		...
		ap_uint<PIXEL_W> shift_reg[64][64];
...
#pragma HLS UNROLL
				if (i == 0) {
					shift_reg[j][0] = buffer_out.read();
				} else {
					shift_reg[j][i] = shift_reg[j][i-1];
				}
				img_out.write(shift_reg[j][63-i]);
			}
		}
...
#pragma HLS PIPELINE II=1
			for (int j = 0; j < 64; j++) {
#pragma HLS UNROLL
				img_out.write( (ap_uint<PIXEL_W>) shift_reg[j][i]);
			}
		}*/
	}
}
...
#pragma HLS DATAFLOW
    ...
    stream<ap_uint<PIXEL_W> > buffer_out("buffer_out_stream");

    bit_reverser(i_in,q_in,i_rev,q_rev,256); // 256

    fft(i_rev,q_rev,fft_out,256); // 256

    averager(fft_out,macro_slice_out,1); // 1

    normalizer(macro_slice_out, buffer_out, 1); // 1
/*
...
#pragma HLS UNROLL
    ...
    image_buffer(buffer_out, img_out, 1); // 1
}
...
#pragma HLS DATAFLOW
	...
	stream<PIXEL_T> image_in;
	stream<PRED_T> pred_out;
	const int out_size = 256;
	const int batch_size = 1; // 1

//	model(i_in, q_in, pred_out, batch_size);
	model(i_in, q_in, pred_out);

	OUT_LOOP_0: for(int i = 0; i < batch_size; i++) {
		OUT_LOOP_1: for (int j = 0; j < out_size; j++) {
#pragma HLS UNROLL
			axis_out tmp_out;
			tmp_out.data = (ap_uint<8>)pred_out.read();
			if((i*out_size+j) == (out_size*batch_size-1)) // j == (out_size-1)
				tmp_out.last = 1;
			else
				tmp_out.last = 0;
